...
RSpec.describe GoogleSerp do
  describe 'Knowledge Card JK Rowlings Books' do
    before :all do
      @data = GoogleSerp.parse('files/extra_results/jk_rowling_books.html').images
      @expected_data = JSON.parse(File.read('files/extra_results/jk_rowling_books_expected_array.json'))
    end
